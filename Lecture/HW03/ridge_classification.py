# MNIST Ridge Classification without library(threshold=0.5)
...
def ridge_classification(X, y):
    ...
    # OLS를 위한 가중치 추정식 계산
    weights = np.linalg.pinv(X_train.T.dot(X_train)+lamb*I).dot(X_train.T).dot(y_train)
...
def ridge_classification_kfold(X, y, n):
...
def main():
    ...
    results = ridge_classification(X, y)
